...
void printSymbol(PCSTR symbolName)
{
	std::unique_ptr<DEBUG_MODULE_AND_ID[]> ids(new DEBUG_MODULE_AND_ID[64]);
	ULONG entriesSize;
	int ret = g_ExtSymbols->GetSymbolEntriesByName(symbolName, 0, ids.get(), 64, &entriesSize);
	dprintf("get %x entries ret:%x\n", entriesSize, ret);
	if (ret == S_OK) {
		char buf[256];
		ULONG nameSize;
		for (int i = 0; i < entriesSize; i++)
		{
			g_ExtSymbols->GetSymbolEntryString(&ids[i], 0, buf, 256, &nameSize);
			dprintf("entry name is :%s\n", buf);
		}
	}	
}
...
cmdsample(PDEBUG_CLIENT4 Client, PCSTR args)
{
	...
	dprintf("arg is:%s\n", args);
	VEC_STR argList;
	boost::split(argList, args, boost::is_any_of(" "), boost::token_compress_on);
	for (auto i : argList)
	{
		dprintf("arg:%s\n", i.c_str());
	}
